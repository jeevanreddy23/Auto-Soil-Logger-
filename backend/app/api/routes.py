...
# --- geologger grid persistence -------------------------------------------
# Simple per-location storage for the static geologger logging grid.
# One JSON payload per location; last write wins (grid is single-user).

def _geologger_table(conn):
    conn.execute("""
        CREATE TABLE IF NOT EXISTS geologger_logs (
...
@router.get("/geologger/logs", dependencies=[Depends(require_api_key)])
async def geologger_list_logs():
    from app.utils.db import get_sqlite_conn
    conn = get_sqlite_conn()
    try:
        _geologger_table(conn)
        cur = conn.execute("SELECT location, payload, updated_at FROM geologger_logs")
        out = {}
        for location, payload, updated_at in cur.fetchall():
            out[location] = {"rows": json.loads(payload), "updated_at": updated_at}
        return {"status": "success", "boreholes": out}
    finally:
        conn.close()


class GeologgerSaveRequest(BaseModel):
    location: str
    rows: list[dict]


@router.post("/geologger/logs", dependencies=[Depends(require_api_key), Depends(rate_limit)])
async def geologger_save_log(req: GeologgerSaveRequest):
    if not req.location or len(req.location) > 64:
    ...
    try:
        _geologger_table(conn)
        conn.execute(
            "INSERT INTO geologger_logs (location, payload, updated_at) VALUES (?, ?, ?) "
            "ON CONFLICT(location) DO UPDATE SET payload=excluded.payload, updated_at=excluded.updated_at",
            (req.location, json.dumps(req.rows), datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()
        return {"status": "success", "location": req.location, "row_count": len(req.rows)}
    except HTTPException:
        raise
    except Exception as e:
        conn.rollback()
