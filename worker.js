...
/* ---- client share links: KV "share:{token}" → {project, ts}; public read-only progress page ---- */
...
async function handleShareView(token, env) {
  const gone = () => new Response("<!DOCTYPE html><h1 style='font-family:sans-serif'>Link expired or revoked</h1>",
    { status: 404, headers: { "Content-Type": "text/html; charset=utf-8" } });
  const raw = await env.GEOFLOW.get("share:" + String(token).slice(0, 64));
  if (!raw) return gone();
  let project; try { project = JSON.parse(raw).project; } catch (e) { project = null; }
